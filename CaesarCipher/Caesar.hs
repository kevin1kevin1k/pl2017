...
histogram xs =
  map (\c -> fromIntegral (count c xs) / num_lowers) alphas
  where alphas = ['a'..'z']
        num_lowers = fromIntegral (lowers xs)
...
crack xs = pos (minimum similarities) similarities
  where hist = histogram xs
        hists = map (\i -> rotate i hist) [0..25]
        similarities = map (chisqr table) hists
        
  -- and decode the input string.

decode :: String -> String
decode xs = encode (26 - crack xs) xs
